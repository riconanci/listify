...
export default function JobsClient({ jobs, searchParams = {} }: JobsClientProps) {
  ...
  const filters = {
    ...
    schedule: (searchParams.schedule as string) || undefined,
    employmentType: (searchParams.employmentType as string) || undefined,
    compModel: (searchParams.compModel as string) || undefined,
    apprenticeFriendly: parseBool(searchParams.apprenticeFriendly),
    posted: (searchParams.posted as string) || undefined,
    address: (searchParams.address as string) || undefined,
    lat: parseFloat(searchParams.lat),
    lng: parseFloat(searchParams.lng),
    radius: parseFloat(searchParams.radius) ?? 15,
  };

  const withinPosted = (dt: string | Date, window?: string) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Column header (tablet/desktop). Matches JobCard grid exactly - UPDATED LAYOUT */}
      <div className="hidden md:grid grid-cols-[140px_2fr_120px_140px_120px_150px_1fr] gap-3 px-3">
